...
async def debug_paths(request):
    current_dir = os.getcwd()
    index_path = os.path.abspath('./public/index_v2.html')
    response_text = (
        f"Current working directory: {current_dir}\n"
        f"Absolute path to index_v2.html: {index_path}\n"
        f"Does index_v2.html exist: {os.path.exists(index_path)}\n"
    )
    print(response_text)
...
async def main():
    ...
    # /public/xterm.css や /public/main.js などでアクセス可能にする
    app.router.add_static('/public', './public', show_index=False) 

    # ルートパスへのアクセス時に index_v2.html を返すハンドラ
    async def index_handler(request):
        print(f"Serving / request. Current dir: {os.getcwd()}")
        index_file_path = './public/index_v2.html'
        if os.path.exists(index_file_path):
            print(f"index_v2.html found at: {os.path.abspath(index_file_path)}")
            return web.FileResponse(index_file_path)
        else:
            print(f"index_v2.html NOT found at: {os.path.abspath(index_file_path)}")
            return web.Response(text="index_v2.html not found!", status=404)

    app.router.add_get('/', index_handler)

    # 明示的に /index.html でも index_v2.html を返す
    app.router.add_get('/index.html', index_handler)

    # デバッグ用パスチェッカー
    app.router.add_get('/check-paths', debug_paths)
